...
def check_winner(board):
    ...
    if len(empty_positions) < 5:
        # 检查所有可能形成五连的路径
        has_winning_path = False
        
        # 检查所有水平路径
        for u in range(rows):
            for v in range(cols - 4):
                path = board[u, v:v+5]
                if 0 in path:  # 路径包含空位
                    if 1 not in path or -1 not in path:  # 路径没有被对方完全阻断
                        has_winning_path = True
        
        # 检查所有垂直路径
        for u in range(rows - 4):
            for v in range(cols):
                path = board[u:u+5, v]
                if 0 in path:
                    if 1 not in path or -1 not in path:
                        has_winning_path = True
        
        # 检查所有对角路径 (右下方向)
        for u in range(rows - 4):
            for v in range(cols - 4):
                path = [board[u+i, v+i] for i in range(5)]
                if 0 in path:
                    if 1 not in path or -1 not in path:
                        has_winning_path = True
...
def computer_move(board, turn):
    ...
    while time.time() - start_time < time_limit:
        ...
        # 检查当前节点是否一开始就已经分出胜负
        if node.move and quick_check_win_at_move(sim_board, node.move, node.player):
            winner = node.player
        ...
        if winner == 0:
            ...
            while depth < max_depth and candidates:
                sim_turn = -sim_turn  # 切换到模拟中的当前行动方
                ...
                for move in candidates:
                    ...
                    if quick_check_win_at_move(sim_board, move, sim_turn):
                        chosen_move = move
                        # 这里的 board 不用复原，因为决定就下这了，且后面会 break
                        break
                    sim_board[move] = 0  # 复原，尝试下一个

                # 2. 如果不能赢，检查是否必须“防守” (对手一步赢)
                if not chosen_move:
                    opponent = -sim_turn
                    for move in candidates:
                        sim_board[move] = opponent
                        if quick_check_win_at_move(sim_board, move, opponent):
                            chosen_move = move  # 必须堵这里
                            sim_board[move] = 0  # 先复原，下面统一落子
                            break
                        sim_board[move] = 0

                # 3. 如果既没杀也没救，再随机
                if not chosen_move:
                    # 简单优化：优先移除列表里随机的一个，效率比重新生成高
                    idx = random.randint(0, len(candidates) - 1)
                    chosen_move = candidates[idx]

                # --- 执行落子 ---
                # 确保该位置是空的（candidates逻辑保证了，但双重保险）
                if sim_board[chosen_move] == 0:
                    sim_board[chosen_move] = sim_turn

                    # 只有当我们是因为“绝杀”选的点，或者运气好随机到了胜点，才判定赢
                    if quick_check_win_at_move(sim_board, chosen_move, sim_turn):
                           winner = sim_turn
                           break

                      # 从候选列表中移除已下的点
                    if chosen_move in candidates:
                        candidates.remove(chosen_move)
                depth += 1
